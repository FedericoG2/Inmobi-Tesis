-- =============================================================================
-- INMOBI — Cálculo ICL en Postgres (UP) — Paso 2
-- Helpers: icl_valor_en_fecha | RPC: calcular_aumentos_pendientes
-- =============================================================================

BEGIN;

-- Valor ICL en p_fecha o el último disponible <= p_fecha (calibración ARquiler: ajustar acá).
CREATE OR REPLACE FUNCTION public.icl_valor_en_fecha(p_fecha date)
RETURNS numeric
LANGUAGE sql
STABLE
SECURITY DEFINER
...
REVOKE ALL ON FUNCTION public.icl_valor_en_fecha(date) FROM PUBLIC;

-- Propuestas de aumento ICL (no persiste; solo lectura contratos + indices).
